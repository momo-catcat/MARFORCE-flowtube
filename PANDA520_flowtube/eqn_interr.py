...
def eqn_interr(num_eqn, eqn_list, chem_scheme_markers):
    ...
    for eqn_step in range(num_eqn[0]):
        ...
        eqn_start = str('.*\\' + chem_scheme_markers[-1])
        rrc_start = str('.*\\' + chem_scheme_markers[10])
        ...
        if eqn_sec == 1:
            ...
            eqn_markers = str('\\' + chem_scheme_markers[-1] + '.*\\' + chem_scheme_markers[10])
        ...
        rate_coeff_start_mark = str('\\' + chem_scheme_markers[10])
        # . means match with anything except a new line character, when followed by a *
        # means match zero or more times (so now we match with all characters in the line
    ...
    return (rindx_g, rstoi_g, pindx_g, pstoi_g, reac_coef_g,
            nreac_g, nprod_g, comp_namelist, comp_num)
