...
def model_1(R2,  Rgrid, Zgrid,  L2, L1,  numLoop, comp_namelist, key_spe_for_plot, dt, timesteps,
    ...
    def animate_func(num,xdata=[],ydata=[],cdata=[]):
        plt.cla()
        plt.close()
        #axs = plt.subplots(math.ceil((len(plot_spec) / 3)), 3, figsize=(9, 5), facecolor='w', edgecolor='k')
        fig,axs = plt.subplots(math.ceil((len(plot_spec)/3)), 3, figsize=(10,5), facecolor='w', edgecolor='k')
        ...
        axs = axs.ravel()
        count = 0
        tim = (num + 1) * timesteps * dt
        xdata.append(np.linspace(0, L1 + L2, Zgrid))
        ydata.append(np.linspace(-R2, R2, Rgrid))
        cdata.append(d[num])
        for i in range(len(plot_spec)):
            cl = axs[i].pcolor(xdata[0], ydata[0], cdata[0][:,:,comp_plot_index[i]],
    ...
    line_ani = animation.FuncAnimation(fig, animate_func, interval=700, frames=j)
    plt.show()
    #plt.close()
        #if (j > 5) & (np.sum(new - old) / np.sum(old) < 1e-5):
        #    break
