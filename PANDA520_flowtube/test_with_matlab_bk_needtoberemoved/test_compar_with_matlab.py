...
#%
    ...
    for j in range(numLoop):
        ...
        c = odesolve(timesteps, Zgrid, Rgrid, dt, D, R,  Q, c, comp_namelist,dydt_vst,rindx,nreac,rstoi,rate_values,L)
...
for i in range(H2SO4.size):#range(H2SO4.size):
    H2SO4[i],HO2[i]= cmd_calib5(O2conc[i], H2Oconc[i],  SO2conc[i], ID / 10 / 2, L / 10, Q * 1000 / 60,  It, T, p, fullOrSimpleModel, time)
