...
# %%import packages
def odesolve(timesteps, Zgrid, Rgrid, dt, D, R, Q, c, comp_namelist, dydt_vst, rindx, nreac, rstoi, rate_values, L):
    import numpy as np
    initc = c
    ...
    for m in range(timesteps):
        ...
        p_b = (initc[2:Rgrid // 2 + 1, 1:-1, :] - 2. * initc[1:Rgrid // 2, 1:-1, :] + initc[0:Rgrid // 2 - 1, 1:-1,
                                                                                      :]) / (dr ** 2)

        p_c = (initc[1:Rgrid // 2, 2:, :] - 2. * initc[1:Rgrid // 2, 1:-1, :] + initc[1:Rgrid // 2, 0:-2, :]) / (
                    dx * dx)
        ...
        term2[1:Rgrid // 2, 1:-1, :] = (2. * Q) / (np.pi * R ** 4) * \
                                       ...
                                       (initc[1:Rgrid // 2, 1:-1, :] - initc[1:Rgrid // 2, 0:-2,
                                                                       :]) / dx  # carried by main flow
        ...
        p_b_end = (initc[2:Rgrid // 2 + 1, -1, :] - 2. * initc[1:Rgrid // 2, -1, :] + initc[0:Rgrid // 2 - 1, -1,
                                                                                      :]) / (dr ** 2)

        p_c_end = (initc[1:Rgrid // 2, -1, :] - 2. * initc[1:Rgrid // 2, -2, :] + initc[1:Rgrid // 2, -2, :]) / (
                    dx * dx)
        ...
        term2[1:Rgrid // 2, -1, :] = (2. * Q) / (np.pi * R ** 4) * \
                                     (R ** 2 - r[1:Rgrid // 2, -1, :] ** 2) * \
                                     (initc[1:Rgrid // 2, -1, :] - initc[1:Rgrid // 2, -2,
                                                                   :]) / dx  # carried by main flow
...
def cmd_calib5(O2conc, H2Oconc, SO2conc, R, L, Q, It, T, p, fullOrSimpleModel, time):
    ...
    eqn_list, aqeqn_list, num_eqn, rrc, rrc_name, RO2_names, eqn_list_on = sch_interr.sch_interr(total_list_eqn,
                                                                                                 chm_sch_mrk)
    ...
    [rindx, rstoi, pindx, pstoi, reac_coef, nreac, nprod, jac_stoi, jac_den_indx, njac_g, jac_indx,
     ...
     rr_arr, rr_arr_p, comp_namelist, comp_list, Pybel_objects, comp_num, RO_indx] = eqn_interr.eqn_interr(num_eqn,
                                                                                                           eqn_list,
                                                                                                           aqeqn_list,
                                                                                                           chm_sch_mrk,
                                                                                                           comp_name,
    ...
    [y, y_mw, num_comp, M, y_indx_plot, dydt_vst,
     comp_namelist, erf, err_mess, NOi, HO2i, NO3i] = init_conc.init_conc(comp_num, Comp0, C0, temp,
                                                                          C0[comp_namelist.index('H2O')], Press,
                                                                          Pybel_objects,
                                                                          testf, dydt_trak, rindx, pindx, num_eqn[0],
                                                                          nreac, nprod, comp_namelist, Compt,
                                                                          comp_namelist, comp_smil, comp_namelist,
                                                                          RO2_indx, HOMRO2_indx, rstoi, pstoi)
    y = c[0, 0, :]
    import RO2_conc
    import rate_coeffs
    RO2conc = RO2_conc.RO2_conc(RO2_indices, y)
    rate_values, erf, err_mess = rate_coeffs.evaluate_rates(RO2conc, y[comp_namelist.index('H2O')], T, 0, M, M * 0.7809,
                                                            y[comp_namelist.index('O2')], 0,
                                                            y[comp_namelist.index('HO2')], 0, p)
    R = 24 / 10 / 2
    Q = 11 / 1000 * 60
    L = 93 * 10
    # %
    ...
    for j in range(numLoop):
        ...
        c = odesolve(timesteps, Zgrid, Rgrid, dt, D, R, Q, c, comp_namelist, dydt_vst, rindx, nreac, rstoi, rate_values,
                     L)
        ...
        for i in range(5):
            ...
            # plt.pcolor(np.linspace(0, L, Zgrid),np.linspace(-R1, R1, Rgrid), c[:, : , order[i]], shading = 'nearest')
            # plt.pcolor(c[:, : , i])
            # colorbar
            plt.xlabel('L [cm]')
            plt.ylabel('r [cm]')
        ...
        meanWeightedH2SO4 = 4 * 0.001 / R ** 2 * np.sum(
            cVec * rVec * (1 - rVec ** 2 / R ** 2))  # not sure about the formulation. Needs to be checked

        meanHO2 = 2 * 0.001 / R ** 2 * np.sum(cVec_HO2 * rVec)  # calculate average concentration over the cross section
        meanWeightedHO2 = 4 * 0.001 / R ** 2 * np.sum(
            cVec_HO2 * rVec * (1 - rVec ** 2 / R ** 2))  # not sure about the formulation. Needs to be checked
...
O2inAir = 0.209
WaterFlow = np.array([50])  # H2O_data['H2O_cali'] #np.array([50, 100, 200]) #[13:19].reset_index()
...
for i in range(H2SO4.size):  # range(H2SO4.size):
    H2SO4[i], HO2[i] = cmd_calib5(O2conc[i], H2Oconc[i], SO2conc[i], ID / 10 / 2, L / 10, Q * 1000 / 60, It, T, p,
                                  fullOrSimpleModel, time)
