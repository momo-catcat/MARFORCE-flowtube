function meanWeightedH2SO4=cmd_calib1Matlab(O2conc,H2Oconc,SO2conc,R,L,Q,It,T,p,fullOrSimpleModel,t1,t2) 
    ...
    numLoop = (t1+t2)/dt/timesteps;                       % number of times to run, a higher number than 1 will plot intermediate results
    numLoop = floor(numLoop) ;                      % number of times to run, a higher number than 1 will plot intermediate results

%     timesteps = L/1000 * pi * (ID/1000).^2/4/(Q / 1000 / 60) / dt;
...
%     end
    ...
    % since there are two different tube with different diameters
    % numLoop =32 is the first part for 58.5 cm, then the left is the 41 cm
    % for 3/4 inch, here the numLoop is get from when we run firstly
    % therefore 
...
%     subplot(2,3,6);
%     plot(0:0.01:R,ppval(splineres, 0:0.01:R));
%     title('[HO2] at end of tube');
%     xlabel('r [cm]');
%     ylabel('Concentration, [cm^{-3}]');
% 
%     rVec=0:0.001:R;
%     cVec=ppval(splineres, 0:0.001:R);
%     meanHO2=2*0.001/R^2*sum(cVec.*rVec);
%     meanWeightedHO2=4*0.001/R^2*sum(cVec.*rVec.*(1-rVec.^2/R^2))
%     % SO3
%     R=R1;
%     dr = R / (Rgrid - 1) * 2;
%     x = R:-dr:0;
%     y = c{2}(1:size(c{2},1)/2,end)';
%     global splineres % this is to be used by f-function
%     splineres = spline(x, y);
% 
%     % plot concentration profile
